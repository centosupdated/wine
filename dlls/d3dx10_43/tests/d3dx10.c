        ...
        unsigned int sub_resource_idx, struct resource_readback *rb)
{
    ...
    switch (d)
    {
        case D3D10_RESOURCE_DIMENSION_BUFFER:
            assert(0 && "Need to import get_buffer_readback() from d3d10core.");
            return;

        case D3D10_RESOURCE_DIMENSION_TEXTURE1D:
            assert(0 && "Need to import get_texture1d_readback() from d3d10core.");
            return;
...
static void test_dxt10_dds_header_image_info(void)
{
    ...
    } tests[] =
    {
         ...
         */
        {
            ...
            { E_FAIL }
#else
            { S_OK, 4, 4, 1, 1, 1, 0, DXGI_FORMAT_R8G8B8A8_UNORM, D3D10_RESOURCE_DIMENSION_TEXTURE2D, }, .todo_hr = TRUE
#endif
        },
        /*
...
static void test_image_filters(void)
{
    ...
    test_filters[] =
    {
        { D3DX10_FILTER_POINT, "D3DX10_FILTER_POINT" },
        { D3DX10_FILTER_LINEAR, "D3DX10_FILTER_LINEAR" },
        { D3DX10_FILTER_TRIANGLE, "D3DX10_FILTER_TRIANGLE" },
        { D3DX10_FILTER_BOX, "D3DX10_FILTER_BOX" },
    };
    static const struct
    {
    ...
    for (i = 0; i < ARRAY_SIZE(tests); ++i)
    {
        ...
        if (tests[i].src_depth > 1)
        {
            set_dxt10_dds_header(&dds.header, DDS_DEPTH, tests[i].src_width, tests[i].src_height, tests[i].src_depth,
                    1, src_pitch, 0, 0);
            set_dds_header_dxt10(&dds.dxt10, tests[i].format, D3D10_RESOURCE_DIMENSION_TEXTURE3D, 0, 1, 0);
        }
        else
        {
            set_dxt10_dds_header(&dds.header, 0, tests[i].src_width, tests[i].src_height, 0, 1, src_pitch, 0, 0);
            set_dds_header_dxt10(&dds.dxt10, tests[i].format, D3D10_RESOURCE_DIMENSION_TEXTURE2D, 0, 1, 0);
        }
        ...
        for (j = 0; j < ARRAY_SIZE(test_filters); ++j)
        {
            ...
            hr = D3DX10CreateTextureFromMemory(device, &dds, dds_size, &load_info, NULL, &resource, NULL);
            ok(hr == S_OK, "Got unexpected hr %#lx.\n", hr);
