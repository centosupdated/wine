...
static ULONG WINAPI memory_buffer_Release(IMFMediaBuffer *iface)
{
    ...
    if (!refcount)
    {
        ...
        if (buffer->d3d12_surface.resource)
        {
            ...
            if (buffer->d3d12_surface.allocator) ID3D12CommandAllocator_Release(buffer->d3d12_surface.allocator);
...
static HRESULT d3d12_surface_buffer_copy_transfer_resource(struct buffer *buffer, MF2DBuffer_LockFlags flags)
{
    ...
    if (!buffer->d3d12_surface.allocator)
    {
        hr = ID3D12Device_CreateCommandAllocator(buffer->d3d12_surface.device, D3D12_COMMAND_LIST_TYPE_COPY,
            &IID_ID3D12CommandAllocator, (void **) &buffer->d3d12_surface.allocator);
        if (FAILED(hr))
            return hr;
    }
    ...
    if (!buffer->d3d12_surface.list)
    {
        hr = ID3D12Device_CreateCommandList(buffer->d3d12_surface.device, 0, D3D12_COMMAND_LIST_TYPE_COPY,
            buffer->d3d12_surface.allocator, NULL, &IID_ID3D12GraphicsCommandList, (void **) &buffer->d3d12_surface.list);
        if (FAILED(hr))
            return hr;
    }

    if (flags != MF2DBuffer_LockFlags_Write && !buffer->d3d12_surface.fence)
    {
    ...
    ID3D12GraphicsCommandList_Reset(buffer->d3d12_surface.list, buffer->d3d12_surface.allocator, NULL);
