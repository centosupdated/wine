...
void info_win32_system(void)
{
    ...
    if (sysinfo.guest_machines[0] != IMAGE_FILE_MACHINE_UNKNOWN)
    {
        ...
        i = 0;
        do
            dbg_printf(" %s", get_machine_str(sysinfo.guest_machines[i]));
        while (sysinfo.guest_machines[++i] != IMAGE_FILE_MACHINE_UNKNOWN);
        dbg_printf(")");
    }
    dbg_printf("\n");

    dbg_printf( "    Version: %s\n", sysinfo.windows_version);
    if (sysinfo.host_system) dbg_printf( "    Host system: %s\n", sysinfo.host_system );
    if (sysinfo.host_version) dbg_printf( "    Host version: %s\n", sysinfo.host_version );
}
